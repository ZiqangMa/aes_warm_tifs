...
Moreover, we approximately calculated $P_{evict}$ under different workloads.
 When {\scshape{Eviction}} event occurs, the encryption time will be greater than $T_{nm}$.
  ...
  So, we use the number of the AES encryption whose execution time is greater than $T_{nm}$ to approximate $P_{evict}$, of which the value is a little larger than $P_{evict}$.
