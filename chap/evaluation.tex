...
Figure~\ref{pic:sec} shows the distributions of AES execution time for two cases. It is clear that most of the execution time is less than $T_{nm}$ or no less than $T_{mm}$ using the {\scshape{Warm+Delay}} scheme.
...
We compare conditional {\vwarm} with  {\vwarm} strategies which have different $P_{1}$ and $P_{2}$.
...
Figure~\ref{pic:eval} validates that the performance of conditional {\scshape{Warm}} strategy is the best.
