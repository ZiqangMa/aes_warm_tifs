...
With {\scshape{Warm+Delay}},
    almost all results between $T_{nm}$ and $T_{mm}$ are delayed to the value greater than $T_{mm}$.
The minimum value of {\scshape{Warm+Delay}} scheme is greater than unprotected encryption because of the cost of {\scshape{GetTime}} (line 2 and 4 in Algorithm \ref{alg:aesdefense}).
The actual maximum value is also greater than $T_{mm}$ for the same reason.
...
Fig. \ref{pic:eval} confirms that the performance of conditional {\scshape{Warm}} is always the best,
    in any case.
...
{\scshape{Warm+Delay}} is compared with other timing side-channel countermeasures:
...
With {\scshape{Warm+Delay}}, the throughput is 23.5 requests per second for 1MB data.
   It is clearly that {\scshape{Warm+Delay}} scheme has a little influence on the performance.
...
Meanwhile,     in Fig. \ref{fig:apacheperformance}, {\scshape{Warm+Delay}} is
    better than other countermeasures except AES-NI.
