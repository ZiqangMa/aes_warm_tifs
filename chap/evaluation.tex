...
With {\scshape{Warm+Delay}},
    almost all the results are in the range of (0, $T_{nm} + 2T_{GetTime}$] and [$T_{mm}+2T_{GetTime}$, $\infty $),
        where $T_{GetTime}$ is the cost of \verb+RDTSCP+ and {\scshape{GetTime}} is called at least twice (Lines 2 and 4 in Algorithm \ref{alg:aesdefense}).
