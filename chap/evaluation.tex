...
Figure~\ref{pic:sec} shows the distributions of AES execution time for two cases. It is clearly that most of the execution time is less than $T_{nm}$ or no less than $T_{mm}$ using the {\scshape{Warm+Delay}} scheme.
...
Moreover, we approximately calculated $P_{evict}$ under different workloads.
 When {\scshape{Eviction}} event occurs, the encryption time will be greater than $T_{nm}$.
  ...
  So, we use the number of the AES encryption whose execution time is greater than $T_{nm}$ to approximate $P_{evict}$, which value is a little larger than $P_{evict}$.
