...
In this section,
 we first demonstrate the result that with our scheme the distribution of AES execution time are separated into two parts: less than TWOCM and no less than WET,
...
Figure~\ref{pic:sec} shows the distributions of AES execution time for two cases. It is clearly that most of the execution time is less than TWOCM or no less than WET using the {\scshape{Warm+Delay}} scheme.
  The time between TWOCM and WET in unprotected AES distribution is delayed to WET.
...
\begin{figure}[t]
  ...
  \includegraphics[width=0.6\textwidth]{pic/ori_scheme_distri.pdf}\\
...
Moreover, we calculated $P_{evict}$ under different workloads,  according to  the number of AES  encryption whose execution time  is greater than TWOCM. %This value represent the $P_{evict}$.
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\textwidth]{pic/performance_comparison.pdf}\\
...
Furthermore, we evaluate the performance of our scheme with different defense methods: AESNI, bitsliced AES implementation, compact table implementation.
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\textwidth]{pic/diff_scheme_compare.pdf}\\
