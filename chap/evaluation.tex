...
Moreover, we approximately calculated $P_{e}$ under different workloads.
 When {\scshape{Eviction}} event occurs, the encryption time will be greater than $T_{nm}$.
  ...
  So, we use the number of the AES encryption whose execution time is greater than $T_{nm}$ to approximate $P_{e}$, of which the value is a little greater than $P_{e}$.
The approximate value of $P_{e}$ is in Table~\ref{tbl:pevict}.
