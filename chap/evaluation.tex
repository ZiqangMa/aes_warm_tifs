...
In this section,
 we first demonstrate the result that with our scheme the distribution of AES execution time are separated into two parts: less than $T_{nm}$ and no less than $T_{mt}$,
...
Figure~\ref{pic:sec} shows the distributions of AES execution time for two cases. It is clearly that most of the execution time is less than $T_{nm}$ or no less than $T_{mt}$ using the {\scshape{Warm+Delay}} scheme.
  The time between $T_{nm}$ and $T_{mt}$ in unprotected AES distribution is delayed to $T_{mt}$.
...
We compare conditional {\vwarm} with  {\vwarm} strategies which have different $P_{1}$ and $P_{2}$.
 The runtime environment is with low workload, high computing workload and high memory workload separately when the interval of OS scheduler is $1$ ms and $4$ ms respectively.
