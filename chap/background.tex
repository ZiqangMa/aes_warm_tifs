...
There are implementations of other block ciphers,
  consisting of table lookup operations and basic computations without data-dependent branches in the execution path,
...
\subsubsection{Cache Timing Side Channel}
...
Two typical remote cache side channels on block cipher implementations are outlined as follows.
...
It statically analyzes the relation between the overall execution time and  lookup table indexes.
...
%
This attack is extended to the second and last rounds of AES~\cite{Ac2007Cache} to extract the full secret key. %without extra information.
It also works for DES and 3DES \cite{Tsunoo2003Cryptanalysis}.

\subsubsection{Active Cache Side Channel}
In the local environment if the caches are shared between attackers and victims,
 the attackers can actively control the cache states and monitor the cache access patterns.
Thereby the keys are derived from the access patterns, called access-driven cache attacks.
...
These active attacks are divided into two categories: synchronous and asynchronous attacks \cite{Osvik2006Cache}.
In a synchronous attack, %the attacker runs serially with the victim.
 the attacker first sets the cache states, then triggers the encryption, and monitor the memory access pattern by detecting the change of cache states after the encryption.
While in an asynchronous attack, the attacker sets the cache states and detects the changes by the victim process running concurrently.
 The synchronous attacks monitor the cache state once per encryption,
 while the asynchronous attacks monitor it for several times per encryption with higher precision.

In an Evict+Time attack \cite{Osvik2006Cache,Tromer2010Efficient},
    which is a typical synchronous attack,
the attacker first triggers the encryption and evicts some certain memory addresses from caches
 to load the data needed by the encryption into caches and to make the addresses to be monitored out of caches.
  Thus the cache states are set.
   Then by triggering the encryption again, the attacker records the encryption time and infers whether the addresses are accessed.
...
Another attack is Prime+Probe \cite{liu2015last,Tromer2010Efficient} which can be used in both scenarios.
The attacker sets the cache states by filling one cache set with known data (Prime).      % 到了什么状态？
It accesses the data, determines whether the data are accessed before through the access time (Probe).
 Through monitoring different cache sets, it obtains the cache access pattern for a synchronous attack, 
  while it is enough to monitor one cache set access pattern for an asynchronous attack.

Flush+Reload \cite{flushreload} is an asynchronous method, where the system supports shared memory.
   Using the attack, a spy program flushes the memory addresses to be monitored out of caches to set the cache in an empty state (Flush),
    then waits a minute and read access these memory addresses to find whether bo be accessed before through accessed time (Reload).
...
Other active attacks like Flush+Flush~\cite{gruss2016flush+flush} and Prime+Abort~\cite{disselkoen2017prime+abort}
 work in asynchronous mode and exploit different methods to observe the cache access patterns.
 Flush+Flush is the variant of Flush+Reload, where recording the time of flush instead of the time of accessing memory.
Prime+Abort protects the data of the Prime phase in Intel TSX.
When it is accessed by the victim, an abort occurs. So the attacker obtains the cache access pattern through the abort.
 This attack replaces the Probe phase with the TSX abort.
...
\subsection{Control against Cache Timing Side Channels}
Different defense methods against the remote cache timing attacks have been proposed on the  levels of hardware, software and operating system (OS).
These defense methods eliminate the execution time difference, % of cache hits/misses,
    ...
    or limit the cache sharing.
 Some of them also are effective against the active attacks.

\noindent\textbf{Eliminating the execution time difference.}
The most direct method to defend against the cache side-channel attacks is to avoid using caches. Page suggests to disable caches in paper~\cite{Page2003Defending}, which now is unrealistical.
Exploiting cache no-fill mode~\cite{Osvik2006Cache,Taha2014Cache} and loading lookup tables into registers~\cite{jayasinghe2010remote,Osvik2006Cache} can avoid the usage of caches, but they suffer from low performance and effect other processes running concurrently.
Meanwhile, several implementations without lookup tables are proposed. AES-NI is a widely used and useful method to resist the cache attacks, which is an hardware implementation introduced by Intel. Bitsliced implementations~\cite{kasper2009faster,K2008A,Osvik2006Cache} of AES based on software can effectively defend against the cache timing attacks. However, they are application specific and hard to design. Besides, the software implementations have significant low performance compared with using lookup tables.
...
Cache warm is another way to eliminate cache misses~\cite{Osvik2006Cache,Page2003Defending,Tsunoo2003Cryptanalysis}, which means to load all the lookup tables into caches before the encryption. Using a compact table instead also can reduce the cache misses~\cite{Gullasch2011Cache,Osvik2006Cache}. Both the two methods would introduce some overload. Furthermore, they cannot avoid all the cache misses but just reduce them to a certain extent.

\noindent\textbf{Confusing the cache accesses.}
Adding delay is a common way to make the attackers obtain the confusing time information. The delay can be added in several ways such as adding random delay~\cite{Osvik2006Cache,Page2003Defending}, and adding dynamic delay to make the encryption time constant~\cite{Askarov2010Predictive,Cock2014The,Ferdinand2004Worst,Zhang2011Predictive}.
  Besides these software methods, OS level defense methods are also proposed such as adding noise with periodic cache cleaning~\cite{Yinqian2013D},
  and making encryption time to constant values by adding delay~\cite{Cleemput2012Compiler,Coppens2009Practical}.
   Using instruction-based scheduling~\cite{Cock2014The,stefan2013eliminating,varadarajan2014scheduler} is other way to confuse the cache accesses.
    These methods are algorithm independent and can effectively defend against the cache side attacks. But the major drawback of these methods is that they incur large performance overhead.
...
In paper \cite{Page2003Defending}, the author suggests to add some dummy instructions or access some extra arrays to confusion the encryption time. Rescheduling the instructions to confuse the cache misses is carried out in both software level~\cite{jayasinghe2012constant,Page2003Defending} and OS level~\cite{Crane2015Thwarting}. The masking technique can be used to the cache attack-resistant algorithm implementations~\cite{Bl2005Provably,Osvik2006Cache}. In addition, a modified random permutation table method raised in paper~\cite{Mer2007Analysis}, and a new cache design called random permutation cache(RPC) confuses the cache misses to the attackers. The combination of blinding and delay is used in paper \cite{K2009A} to reach the goal of confusion. These methods need modifications to the existing implementations making them suffer from the performance problem while have limited practical usage.

Another method to confuse the attackers' observations is modifying the precision of the time measured
 by attackers \cite{Li2013Mitigating,Martin2012TimeWarp,Page2003Defending}.
But it is useless in the remote environment because the attackers can use its own timers.

\noindent\textbf{Limiting Cache Sharing.} %Cache独占使用
...
Some schemes integrate several strategies to defend against the cache timing attack. The scheme proposed in \cite{Brickell2006Software} consists of three parts: using compact tables, frequently randomizing tables, and pre-loading of relevant cache-lines. It makes cache misses occur as little as possible while makes the observations secret-independent. But the drawback is the performance overload as a result that three parts all would introduce some extra overhead.
...
\hline
$T_{me}$ & the maximum encryption execution time when all lookup tables are out of caches, without \\        % 定义需要再准确一些！
\hline
...
\hline
$P_{d}$ & the probability that performing {\scshape{Delay}} due to not loading $N$ lines of lookup tables \\
\hline
$X_n$ & the state of the lookup table in the caches \\
\hline
     $S_{c}$&  the state that all entries of the lookup tables are in caches\\
\hline
...
\hline
$T_{d}$ & the extra cost introduced by  {\scshape{Delay}} operation\\
\hline
$T_{w}$ & the extra cost introduced by  {\scshape{Warm}} operation\\
\hline
$T_{w}^{min}$ & the minimum value of $T_{w}$\\
\hline
$G$ & $T_{d}/T_{w}$\\
\hline
$M$ & $T_{w}^{min} / T_{d}$\\
