...
When the caches are shared by attackers and victims on the same system,
 the attackers could actively control the cache states and monitor the cache accesses by the victim process.
Then, the keys are derived from the access patterns, called access-driven attacks.
...
Such active attacks fall into two categories: synchronous and asynchronous attacks \cite{Osvik2006Cache}.
...
the attacker first triggers the encryption to load the data needed into caches 
    and evicts some data from caches, of which the addresses will be monitored.
So the cache states are set.
...
Then, it accesses these data and measures the access time,
 to determines whether some memory data are accessed and cached in the same cache set during the encryption,
 called Probe.
...
The attack process first flushes the shared memory data out of caches (Flush),
    and then measures the access time of some addresses to    
     find whether they are accessed by the concurrent encryption (Reload).
...
%
Flush+Flush~\cite{gruss2016flush+flush} and Prime+Abort~\cite{disselkoen2017prime+abort} are 
