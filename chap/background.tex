...
Typically, it takes 3 to 4 cycles for a read operation in L1 caches,
...
The attacker firstly obtains the AES execution time for different value of $p_i \oplus k_i$ (where $p_i$ and $k_i$ are the $i$th byte of the plaintext and key, respectively) %, and  $0 \leq i < 16$)
     on a duplicated server whose hardware and software configurations are the same as the victim server.
      It finds the value $p_i \oplus k_i$ corresponding to the maximum average AES execution time.
Then for the $i$th byte of the unknown key (denoted as $k_i^{'}$) on the victim server,
 it collects a large number of overall encryption times for different known plaintexts, 
    and obtains $p_i^{'}$ corresponding to the maximum average AES execution time.
The maximum time has the same index in the two stages,
...
This attack is based on the fact that less time is needed for accessing the lookup table entries in the same cache line.
...
Taking the attack  on  first round of AES~\cite{Bonneau2006Cache} as an example,
 on the platform where each cache line of64 bytes has $16$ 4-byte entries,
the inputs of lookup table $T_i$ $(0 \leq i \leq 3)$ are $p_{(i+4j)\%16} \oplus k_{(i+4j)\%16}$ where $0 \leq j \leq 3$.
When any two inputs of $T_i$ require accessing the lookup table entries in a same cache line,
 it results in shorter execution time, which is called an internal collision.
 %Then,
 The attacker  makes a statistics of the average execution time for all possible $p_{(i+4j)\%16} \oplus p_{(i+4j^{'})\%16}$ ($0 \leq j, j^{'} \leq 3$).
 The lowest time at the value $p_{(i+4j)\%16} \oplus p_{(i+4j^{'})\%16}=\delta$ represents that cache collision occurs, which means $k_{(i+4j)\%16} \oplus k_{(i+4j^{'})\%16}=\delta \mod 16$.
   In this way, the higher $4$ bits of secret key are extracted.
...
In the local environment if the caches are shared between attackers and victims, the attackers can actively control the cache states and monitor the cache access patterns.
...
In a synchronous attack, %the attacker runs serially with the victim.
 the attacker first sets the cache states, then triggers the encryption, and detects the change of cache states after the encryption.
While in an asynchronous attack, the attacker sets the cache states and detects the changes by the victim process running concurrently.
 The synchronous attacks monitor the cache once per encryption.
While asynchronous attacks have higher precision which can monitor several times per encryption.
In an Evict+Time attack \cite{Osvik2006Cache,Tromer2010Efficient},
    which is a typical synchronous attack,
the attacker triggers the encryption,
 then evicts the cache by accessing some different memory addresses and finally triggers the encryption again and time it.
%   The attacker repeats these steps and infers the key information from the observations.

Another method is Prime+Probe \cite{liu2015last,Tromer2010Efficient} which can be used in both scenarios.
In Prime phase, the attacker fills the cache with a known array.
While in Probe phase, the attacker records the time of accessing the elements in the identical array.

Flush+Reload \cite{flushreload} is an asynchronous method, where the system supports shared memory.
  A  Flush+Reload attacker monitors other processes reading and executing access to shared memory pages.
   Using the attack, a spy program reads access to the shared memory pages, and learns when the victim executes the code in the
    monitored memory locations.
%     From this information attackers can infer the victim's confidential data.
Active cache  side-channel attacks also include Flush+Flush~\cite{gruss2016flush+flush} and Prime+Abort~\cite{disselkoen2017prime+abort}.
They work in asynchronous mode and utilize different methods to observe the cache access patterns.
