...
Each round transformation consists of \texttt{SubBytes}, \texttt{ShiftRows},
 \texttt{MixColumns} and \texttt{AddRoundKey} on the 128-bit state.\footnote{The last round performs \texttt{SubBytes}, \texttt{ShiftRows} and \texttt{AddRoundKey} only.}
...
Data in caches are loaded and evicted in cache lines.
Each cache line (64 bytes) usually contains multiple lookup table entries,
 and accessing the entries in a same cache line is called an internal collision.
% the internal collision is used to denote accessing the entries in the same cache line.
The internal collision attack works for DES, 3DES and the first round of AES \cite{Tsunoo2002Cryptanalysis,Tsunoo2003Cryptanalysis},
 and is extended to the second and last rounds of AES~\cite{Ac2007Cache}.
 This attack exploits the relationship between the encryption time and the number of cache hits.
Let's take the first round of AES as an example,
...
When any two inputs access the lookup table entries in a same cache line,
 it results in shorter execution time.
 %\footnote{According to \cite{Tsunoo2003Cryptanalysis}, a longer encryption time results for the implementation~\cite{aesimp} of AES running on 600-MHz Pentium III CPUs.}.
{\color{red}Then, the difference of some key bits is disclosed as the values (i.e., $p_{(i+4j)\%16} \oplus p_{(i+4k)\%16}$, $0 \leq j < k \leq 3$) counted most often,}
...
It is based on the case that the whole AES execution time is correlated with the time for accessing the lookup tables, which depends on the lookup table index \cite{Bernstein2005Cache}.
