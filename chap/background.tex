...
In a synchronous attack, the attacker can use Evict+Time \cite{Osvik2006Cache,Tromer2010Efficient}
...
In Prime+Probe, the attackers fill the cache with a known array before trigger the encryption.
...
Another method is Flush+Reload \cite{flushreload}, where the system supports shared memory.
...
Active cache  side-channel attacks include Flush+Flush~\cite{gruss2016flush+flush}, Prime+Abort~\cite{disselkoen2017prime+abort}.
They utilize different methods to observe the cache access pattern and then attempt to retrieve the confidential data
