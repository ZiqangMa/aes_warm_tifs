...
The attacker sets the cache states by filling an array (called Prime).      % 到了什么状态？
...
Flush+Reload \cite{flushreload} is an asynchronous method, where the system supports shared memory.
   Using the attack, a spy program sets the cache states empty by flushing it,
    then read access the shared memory to find the change of cache states and retrieve the memory access pattern by the victim.
...
Prime+Abort protects the array of the Prime phase in Intel TSX.
When it is accessed by victim, an abort occurs % the attacker know where is accessed.
 This attack replaces the Probe phase with TSX abort.
