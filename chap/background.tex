...
These defense methods eliminate the execution time difference, % of cache hits/misses,
    introduce confusion to cache accesses,% to obscure the difference,
...
Meanwhile, several implementations without lookup tables are proposed. AES-NI is a widely used and useful method to resist the cache attacks, which is a hardware implementation introduced by Intel. Bitsliced implementations~\cite{kasper2009faster,K2008A,Osvik2006Cache} of AES based on software can effectively defend against the cache timing attacks. However, they are application specific and hard to design. Besides, the software implementations have significant low performance compared with using lookup tables.
