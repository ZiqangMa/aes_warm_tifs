...
(i.e., the high 4 bits of $p_{i+4j} \oplus k_{i+4j}    \oplus    p_{i+4j'} \oplus k_{i+4j'}$ is zero),
 ...
 The least time at the value $p_{i+4j} \oplus p_{i+4j^{'}}=\delta$ means that the higher 4 bits of $k_{i+4j} \oplus  k_{i+4j^{'}}$ are the same as $\delta$ .
   In this way, the key bytes information are leaked.
