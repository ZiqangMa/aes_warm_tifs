...
Each cache line of 64 bytes has $16$ 4-byte entries,
 and in the first round of AES~\cite{Bonneau2006Cache},
% on the platform where
the inputs of lookup table $T_i$ $(0 \leq i \leq 3)$ are $p_{i+4j} \oplus k_{i+4j}$ where $0 \leq j \leq 3$.
When any two inputs of $T_i$ access the lookup table entries in a same cache line
(i.e., the higher 4 bits of $p_{(i+4j)\%16} \oplus k_{(i+4j)\%16}    \oplus    p_{(i+4j')\%16} \oplus k_{(i+4j')\%16}$ is zero),
 ...
 The least time at the value $p_{i+4j} \oplus p_{i+4j^{'}}=\delta$ means that $\langle k_{i+4j}\rangle \oplus \langle k_{i+4j^{'}}\rangle =\delta$ ($\langle k\rangle$ represents the high $4$ bits of $k$).
   In this way, the information about the high $4$ bits of key bytes are leaked.
