...
Typically, it takes 3 to 4 cycles for a read operation in L1D caches,
 while an operation in RAM takes about 250 cycles \cite{drepper2007every}.
  ...
  Based on the ability of the attacker, cache attacks are divided into three categories: trace-driven, time-driven and access-driven attack.
   In trace-driven attack, the attacker is able to access the physical hardware directly to monitor the electromagnetic fields or power variation.
    And in time-driven attack, the attacker can monitor the encryption execution time.
     While in access-driven attack, the attacker can actively control the cache state so that it can monitor the cache access pattern of the encryption.
      For trace-driven and time-driven attack, the attacker cannot impact the cache activities of the encryption. 
...
\subsubsection{Active Cache Side Channel}
 When the caches are shared by attackers and victims on the same system,
  the attacker could actively control the cache states and monitor the cache accesses by the victim process.
...
This attack falls into two categories: synchronous and asynchronous attack \cite{Osvik2006Cache}.
...
Defense methods against the remote cache timing attacks have been proposed on different levels,
...
The straightforward method to defend against the cache side-channel attacks is to avoid using caches. Page suggests to disable caches in paper~\cite{Page2003Defending}, which now is unrealistical.
...
Meanwhile, several implementations without lookup tables are proposed. Bitsliced implementations~\cite{kasper2009faster,K2008A,Osvik2006Cache} of AES based on software can effectively defend against the cache timing attacks. However, they are application specific and hard to design. Besides, the software implementations have significant low performance compared with using lookup tables.
AES-NI is a widely used and useful method to resist the cache attacks, which is a hardware implementation introduced by Intel.
But it only works for AES and is only available on Intel CPUs.
...
Cache warm is another way to eliminate cache misses~\cite{Osvik2006Cache,Page2003Defending,Tsunoo2003Cryptanalysis}, which means to load all the lookup tables into caches before the encryption. Using a compact table instead also can reduce the cache misses~\cite{Gullasch2011Cache,Osvik2006Cache}. Both the two methods introduce some overload. Furthermore, they cannot avoid all the cache misses but just reduce them to a certain extent.

\noindent\textbf{Confusing the cache accesses.}
Adding delay is a common way to make the attacker obtain the confusing time information. The delay can be added in several ways such as adding random delay~\cite{Osvik2006Cache,Page2003Defending}, and adding dynamic delay to make the encryption time constant~\cite{Askarov2010Predictive,Cock2014The,Ferdinand2004Worst,Zhang2011Predictive} in software level and OS level~\cite{Yinqian2013D,Cleemput2012Compiler,Coppens2009Practical}.
   Using instruction-based scheduling~\cite{Cock2014The,stefan2013eliminating,varadarajan2014scheduler} to replace the original time-based scheduling is another way to confuse the cache accesses.
    These methods are algorithm-independent and can effectively defend against the cache side channel attacks. But the major drawback of these methods is that they incur large performance overhead.
...
In paper \cite{Page2003Defending}, the author suggests to add some dummy instructions or access some extra arrays to confuse the encryption time. Rescheduling the instructions to confuse the cache misses is carried out in both software level~\cite{jayasinghe2012constant,Page2003Defending} and OS level~\cite{Crane2015Thwarting}. The masking technique can be used to the cache attack-resistant algorithm implementations~\cite{Bl2005Provably,Osvik2006Cache}. In addition, a modified random permutation table method raised in paper~\cite{Mer2007Analysis}, and a new cache design called random permutation cache (RPC) confuse the cache misses to the attacker. The combination of blinding and delay is used in paper \cite{K2009A} to reach the goal of confusion. These methods need modifications to the existing implementations making them suffer from the performance problem while have limited practical usage.

Another method to confuse the attacker's observations is modifying the precision of the time measured
 by the attacker \cite{Li2013Mitigating,Martin2012TimeWarp,Page2003Defending}.
But it is useless in the remote environment because the attacker can use its own timer.

\noindent\textbf{Limiting Cache Sharing.} %Cache独占使用
Another way to eliminate cache attacks is cache partition. Cache coloring is an explicit method to partition the cache on OS level~\cite{Cock2014The}. STEALTHMEM~\cite{Kim2012STEALTHMEM} allows each VM to load the sensitive data into its own locked cache lines. A hardware-based mechanism presented in~\cite{Dan2005Partitioned}, allows caches to be configured dynamically to match the need of a process.
The new cache design in \cite{Kong2009Hardware,Wang2007New} uses partition-locked caches to prevent cache interference.
...
limited practical usage and are hard to be deployed on ready-made commodity hardware.

\noindent\textbf{Integrated methods.}
Some schemes integrate several strategies to defend against the cache timing attack. The scheme proposed in \cite{Brickell2006Software} consists of three parts: using compact tables, frequently randomizing tables, and pre-loading of relevant cache-lines. It makes cache misses occur as little as possible while makes the observations secret-independent. But the drawback is the performance overload as all three parts would introduce some extra overhead.

Another scheme is held in \cite{Braun2015Robust}, which employs dynamic padding, isolating shared resources and lazily cleansing state to form a robust defense. It considers the performance problem and decreases the reduction of performance through careful design. But the scheme needs some privileges to modify the OS kernel.
...
\hline
$T_{mm}$ & the maximum encryption execution time when all lookup tables are out of caches, i.e. the maximum number of cache misses occur. \\        % 定义需要再准确一些！
\hline
