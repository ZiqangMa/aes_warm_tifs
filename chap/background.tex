...
Thereby the keys can be derived from the access patterns, called access-driven cache attacks.
Attackers and victims can share either the L1 cache or last-level cache (LLC).
...
These attacks are divided into two categories: synchronous and asynchronous attacks~\cite{Osvik2006Cache}.
In synchronous attacks, the attackers can use Evict+Time \cite{Osvik2006Cache,Tromer2010Efficient}
 or Prime+Probe \cite{liu2015last,Tromer2010Efficient}
    to measure the cache access patterns.
The Evict+Time attackers trigger the encryption,
 then evict the cache by accessing some different memory addresses and finally trigger the encryption again and time it.
...
These synchronous attacks monitor the cache once per encryption.
