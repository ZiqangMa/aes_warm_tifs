...
When any two inputs of $T_i$ access the lookup table entries in a same cache line,
 ...
 %Then,
 The attacker measures the execution time for all possible $p_{(i+4j)\%16} \oplus p_{(i+4j^{'})\%16}$ ($0 \leq j, j^{'} \leq 3$).
 The least time at the value $p_{(i+4j)\%16} \oplus p_{(i+4j^{'})\%16}=\delta$ means that $k_{(i+4j)\%16} \oplus k_{(i+4j^{'})\%16}=\delta \mod 16$ (called an internal collision).
   In this way, the information about the higher $4$ bits of key bytes are leaked. ++++
