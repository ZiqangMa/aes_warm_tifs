...
(i.e., the high 4 bits of $p_{i+4j} \oplus k_{i+4j}    \oplus    p_{i+4j'} \oplus k_{i+4j'}$ is zero),
 ...
 The least time at the value $p_{i+4j} \oplus p_{i+4j^{'}}=\delta$ means that
   $k_{i+4j} \oplus k_{i+4j^{'}}$ has the same high $4$ bits as $\delta$.
    In this way, the information of key bytes is leaked.
...
the attacker sets the cache states by triggering the encryption and accessing some memory addresses.
 by the change of cache states, it can be inferred that whether these addresses are accessed.
...
The attacker sets the cache states by filling a known array (called Prime).
It accesses the identical array to determine the cache states and achieves the cache access pattern (called Probe).

However, Flush+Reload \cite{flushreload} is an asynchronous method, where the system supports shared memory.
   Using the attack, a spy program sets the cache states empty by flushing it, then read access the shared memory to find the change of cache states and retrieve the memory access pattern by the victim.
...
Other active attacks like Flush+Flush~\cite{gruss2016flush+flush} and Prime+Abort~\cite{disselkoen2017prime+abort}
 work in asynchronous mode and utilize different methods to observe the cache access patterns.
 Flush+Flush is the variant of Flush+Reload, where recording the time of flush instead of the time of accessing the memory.
Prime+Abort protects the array of the Prime phase in Intel TSX. When it is accessed by victim, an abort occurs. This attack replaces the Probe phase with
TSX abort.
...
\noindent\textbf{Confusing the execution time.}
