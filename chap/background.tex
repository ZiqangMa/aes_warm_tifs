...
Then for $i$th byte of the unknown key $k_i^{'}$ on the victim server, it collects a large number of overall encryption times for different known plaintexts,  obtains $p_i^{'}$ corresponding to the maximum average AES execution time. 
The maximum time has the same index in the two stage, so the attacker infers $k_i^{'} = p_i^{'} \oplus ( p_i \oplus k_i) $.
...
While in an asynchronous attack, the attacker sets the cache states and detects the changes with the victim running parallel.
The Evict+Time \cite{Osvik2006Cache,Tromer2010Efficient} method is used in a synchronous attack.
 Using this method, the attacker triggers the encryption,
...
Flush+Reload \cite{flushreload} is an asynchronous method, where the system supports shared memory.
  A  Flush+Reload attacker monitors other processes reading and executing access to shared memory pages.
   Using the attack, a spy program reads access to the shared memory pages, and learns when the victim executes the code in the
    monitored memory locations. From this information attackers can infer the victim's confidential data.
...
These defense methods eliminate the execution time difference of cache hits/misses,
    ...
    or both. 
 Some of them also are available for the active attacks.
...
\noindent\textbf{Adding confusion to cache accesses.}
