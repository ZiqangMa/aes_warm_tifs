...
However, when a greater lookup table is used,
 all cache sets will be occupied by default due to the continuity of lookup tables in memory.
 In these cases,
  to avoid the the read-write conflict of cache sets and declare an array as the stack,
   we shall reserve the constant lookup table within 32 cache sets.
It can be achieved by discontinuous table entries.
For example, when using a 4KB lookup table ($T_{0}, T_{1}, T_{2}, T_{3}$),
 we allocate $T_2$ in the same memory address as $T_0$ module 4096, so that they occupy the same cache sets.
Then a 4KB lookup table only occupies 32 cache sets and the 2KB stack occupies the other 32 cache sets.
...
This method works for a 4.25KB or 5KB lookup table. 
...
The {\scshape{Warm+Delay}} scheme is effective against remote cache timing side channels,
    and also works for some active cache attacks.
...
The cache access pattern is masked by the {\scshape{Warm+Delay}} scheme.
%So the {\scshape{Warm+Delay}} scheme is effective for the synchronous attacks.

As for the trace-driven attack, attackers can observe the cache access pattern during the encryption execution,
 so {\scshape{Delay}} is ineffective.
{\scshape{Warm}} is effective,
    but not always executed.
Therefore, {\scshape{Warm+Delay}} mitigates the trace-driven attacks but not eliminate the attacks.
...
Finally, the {\scshape{Warm+Delay}} scheme is ineffective 
 ...
 because the attackers can change the cache states at any time and observe the cache access pattern during encryption.
