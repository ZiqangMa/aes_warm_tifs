...
However, when using greater lookup tables, all cache sets will be occupied by default due to the continuity of lookup tables in the RAM.
...
For the trace-driven attack, attackers can observe the inner cache access pattern of the encryption execution, so {\scshape{Delay}} is not effective.
While {\scshape{Warm}} is effective but not always executed.
 Therefore, the {\scshape{Warm+Delay}} scheme can not eliminate but just mitigates the trace-driven attacks.
%In synchronous attacks, the attacker only monitors the cache pattern for the whole encryption.
%For Evict+time, the attacker
For the access-driven attack, the synchronous attacks can be prevented by the {\scshape{Warm+Delay}} scheme.
