...
The overall execution time %of block ciphers
 is influenced due to the number of cache misses in table lookup.
...
%In particular,
 {\scshape{Warm}} and  {\scshape{Delay}} are two typical algorithm-independent and implementation-transparent mechanisms
    ...
    and
    {\scshape{Delay}} inserts padding instructions \emph{after} the encryption operations.\footnote{Another choice is to perform encryption without using caches, but it is very inefficient.
     In fact, a typical strategy of {\scshape{Delay}}
     is to extend the encryption operation in constant time, equal to the execution time without caches.} With {\scshape{Warm}} and/or {\scshape{Delay}}, the execution time measured by attackers becomes \emph{irrelevant} to the cache misses/hits \emph{during} encryption.
...
In this paper, we investigate the performance of symmetric cryptographic functions that implement {\scshape{Warm}} and {\scshape{Delay}} to defend cache timing side channel attacks,
  and attempt to find \emph{the optimal strategy to integrate these two complementary mechanisms}.
...
The integration scheme does not require any privileged operation on computer systems.
...
\item
We implement the optimal {\scshape{Warm+Delay}} integration scheme on Linux with Intel Core CPUs for AES-128,
  and experimentally confirm that it (\emph{a}) eliminates cache timing side channels,
   (\emph{b}) outperforms other different integration strategies, % of {\scshape{Warm}} and {\scshape{Delay}},
    and
   (\emph{c}) works without privileged operations on the system.
