...
Generally, for a block cipher,
  ...
  In the remainder, we emphasize encryption only, but the descriptions and conclusions are also applicable to decryption.
...
Different methods are proposed against cache timing side channels,
  by destroying the relationship between the secret keys and the execution time.
Intuitively,
     in order to eliminate remote cache-based timing side channels for the implementation of block ciphers,
  the basic algorithm-independent mechanism is
    to perform encryption, \emph{a})  with all lookup tables inside/outside caches,
    or \emph{b}) in a constant period of time.
%    \footnote{Typical values of this constant are, the execution time without any caches, and the time when lookup tables are outside caches and loaded into caches during the encryption.}
...
{\scshape{Warm}} and  {\scshape{Delay}} are two typical algorithm-independent mechanisms,
  have been proposed to prevent cache side-channel attacks~\cite{Osvik2006Cache,Gullasch2011Cache,Bernstein2005Cache,Canteaut2006Understanding,Zhang2011Predictive}.
  \footnote{without caches are another choice, but another goal is efficiency.}
    {\scshape{Warm}} fills cache lines by reading constant lookup tables before the encryption/decryption operations.
       Because all tables are in the cache, the cache misses are eliminated.
   {\scshape{Delay}} inserts padding instructions after the encryption/decryption operations.
     This makes all execution time extend to a constant value so that the cache misses are indistinguishable.
However, the two mechanisms often introduce some extra operations.
 Meanwhile, {\scshape{Warm}} is not always effective due to the existence of interruption,
  and {\scshape{Delay}} will tremendously impact the performance if used only.
...
In this paper, we integrate {\scshape{Warm}} and {\scshape{Delay}} into a mechanism
