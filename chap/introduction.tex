...
So  we emphasize encryption only in the remainder, but the discussions and conclusions are also applicable to decryption.
...
while other active cache-based side-channel attacks involve operations
 by a malicious task sharing caches with the victim cryptographic engine
...
We apply the optimization  integration to AES,
...
when (\emph{a}) the probability
 ...
 (\emph{b}) the ratio of
  the delayed time of padding instructions to the time of reading constant tables {\color{red}from  either caches or RAM,
...
We analyze the performance, as the integration scheme does not require any privileged operation on the computer system.
