...
To launch  an internal collision attack \cite{Bonneau2006Cache,Ac2007Cache},
 the attackers need to determine the minimum execution time for all possible $p_{i+4j} \oplus p_{i+4j^{'}}$ ($0 \leq i \leq 3, 0 \leq j, j^{'} \leq 3$)
  and use $k_i \oplus  k_j = p_i \oplus p_j$ to obtain the key information.
When protected by the {\scshape{Warm+Delay}} scheme,
 the execution time measured by attackers is irrelative to the inputs of the lookup table
  and the time variation is caused by the random and unknown system activities.
   So the minimum time does not mean a collision occurrence, that is $p_i \oplus  k_i = p_j \oplus k_j$ at here does not hold any more.
