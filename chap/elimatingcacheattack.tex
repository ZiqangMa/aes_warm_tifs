...
Then, the {\scshape{Warm+Delay}} scheme is described,
...
We focus on the \emph{remote cache side-channel attacks} on block ciphers.
The algorithms of block ciphers are publicly known, but the keys are kept secret before the attacks are launched.
The implementation details such as source code, operating system, cache hierarchy and other software/hardware configurations,
  are also publicly known;
  but the running states of the target system are unknown  to the  remote attackers
   due to non-deterministic interrupts, task scheduling and other system activities.
In particular,
 the implementation of block ciphers is based on lookup tables,
  and the execution time depends mostly on the cache access of lookup tables.

The attackers are able to
  invoke the encryption function arbitrarily
  and measure the time for each invocation.
The attackers control the invocation,
 so they could continuously invoke the function to cache
        all lookup tables % with a high probability,
    or invoke no function for a long time to let
           all tables be evicted from caches, with an extremely high probability.

We assume an unprivileged attacker that cannot run a processes on the target systems
   to concurrently modify (or probe) the cache state of the system, % (shared with the cryptographic engine)
 and they just measure the overall execution time of the block ciphers.
%In our model, the target operating system and the underlying hardware are regarded as security.
 We expect that attackers do not have physical access to the hardware. % and cannot run any processes on the target as well.

This work focuses on the remote cache timing side-channel attacks,
  which are launched with the least privileges (or capabilities) of attackers;
...
other cache timing side-channels by active attackers {\color{red}(such as xxxx)} are out of the scope of this paper.
...
%
%
We attempt to eliminate remote cache timing side channels,
...
\begin{itemize}
  ...
  The scheme is algorithm-independent and
        works well for different implementations %of block ciphers
...
%  It doesn't need to modify the encryption algorithm code, and can be applied to all the symmetric encryption algorithm implementations based on the lookup tables.
  ...
  All operations introduced by the defense scheme, are available in common computer systems.
   It works well either in user space or kernel space,
     to protect the block cipher implemented in user mode and kernel mode, respectively.
  \item
   The parameters are configurable to optimize the performance.
Then, it produces the optimal performance by configuring appropriate parameters
    for different block-cipher implementations and computing platforms.
\end{itemize}
...
The scheme integrates two basic countermeasures against remote cache timing side channels~\cite{Osvik2006Cache,Gullasch2011Cache,Bernstein2005Cache,Canteaut2006Understanding}:
    \emph{a}) {\scshape{Warm}}, load the lookup tables into caches before encryption;
  and \emph{b}) {\scshape{Delay}},     insert padding instructions after encryption.
...
%  %\item
%\end{enumerate}
We attempt to optimize the performance while ensure the security.
Therefore, we focus on the situation where large amounts of data are encrypted and the speed of encryption matters.

A naive scheme is to perform both {\scshape{Warm}} and {\scshape{Delay}} every time the protected cryptographic function is invoked.
 But it introduces too many extra operations and the performance is seriously degraded, although the security is ensured.
So we perform {\scshape{Delay}} as a backup line of defense,
    or only when the effect of {\scshape{Warm}} is broken and the execution time may leak information on the secret key
         (i.e., is not equal to the expected execution time with all tables cached).
Next,
  when we consider that the cryptographic function is invoked continuously,
%   and before the next execution perform {\scshape{Warm}}.
%    In this way the performance of the scheme is much improved.
...
Finally,
    we find that,
     the conditions to perform {\scshape{Warm}} and {\scshape{Delay}} are similar.
That is,
    when the execution time is greater than the expected time with all tables cached,
        {\scshape{Delay}} is necessary to eliminate the risk and {\scshape{Warm}} is also necessary for the next encryption because probably some table entry is  uncached.
So,
   in our scheme,
   we perform {\scshape{Warm}} as a part of inserted instructions by {\scshape{Delay}},
   if it is needed.
   This design further improves the performance, and the variation of {\scshape{Warm}} is masked.
%     Therefore, the time of {\scshape{Warm}} cannot be detected and we do not need to flush the cache.

%Principles. Warm before function, delay after function.
%As least as possible.
%Conditions are defined as time, no special privileges.
%The same as the timing side-channel attackers.
%We focus on the situation where a mass data are processed.
%A naive scheme, is ; but ...
%then, ... improved;
%\textbf{+++++}
...
$T_{W}$ is defined as the period for one execution of encryption,
  in the case that none of table entries is cached before the execution.
These two parameters are measured when there is no concurrent interrupts, task scheduling or other system activities.
...
In addition to encryption, three operations
...
All these operations are algorithms-independent and implementation-transparent,
        except that the address of lookup tables is needed in {\scshape{Warm()}}.
These operations are supported in commodity computer systems,
...
So {\scshape{Warm()}} takes effect in the next execution of encryption.
Therefore,
...
We ensure that the measured time does not leak any exploitable information about the status of cache lines,
...
\begin{itemize}
  \item \textbf{Case 1}: The execution time of {\scshape{Crypt()}} is in $(0, T_{NM}]$.
   In fact, it is almost a fixed value for arbitrary data processed, as all tables are cached and the time is $T_{NM}$.
  ...
  \item \textbf{Case 3}: The execution time is greater than $T_{W}$.
   ...
   As explained in Section~\ref{sec:assumption},
     the running states including the system activities,
       ...
       so the attackers cannot find the actual execution time of encryption to infer the keys.
\end{itemize}
...
However, protected by the proposed {\scshape{Warm+Delay}} scheme,
  any plaintext is processed in the short time (i.e., $T_{NM}$) due to cache warm,
  while it is done in the long time (i.e., $T_{W}$ or longer) due to system activities after {\scshape{Delay}}.
Therefore, the distribution of the collections depends on the non-deterministic system activities,
   but not the data processed;
 or, the attack results will be random.
...
Since the system activities of the target servers are different from those of the duplicated server
 and unknown to the attackers,
  this pattern cannot be used to infer the keys in the target server.
