...
As the baseline, the defense mechanism shall satisfy the following conditions:

\begin{itemize}
  \item The solution is algorithm-independent.
  It is universally applicable to various block ciphers and implementations that use lookup tables (so that they are vulnerable to cache timing side-channel attacks).
...
\begin{itemize}
  \item We attempt to develop the first %mathematical
   model to formally analyze the performance of block cipher implementations that integrate {\scshape{Warm}} and {\scshape{Delay}} to defend against the cache timing side-channel attacks.
...
%So we focus on the situation where large amounts of data are encrypted and then the speed of encryption matters.
  \item In our model, the parameters are configurable. Therefore, by configuring appropriate parameters, optimal performance
...
A naive scheme is performing both {\scshape{Warm}} and {\scshape{Delay}} operation every time the protected cryptographic function is invoked.
