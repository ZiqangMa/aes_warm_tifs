...
Then, the {\scshape{Warm+Delay}} defense scheme is described,
...
In this paper we consider the \emph{remote cache side-channel attacks} on block ciphers.
The algorithms of block ciphers and their implementation details are publicly known, but the keys are kept secret before the attacks are launched.
In particular,
 the implementation of block ciphers is based on lookup tables,
  and the execution time depends mostly on the cache access of table lookup.

The remote attackers know the operating system, cache hierarchy and other software/hardware configurations on the target systems,
 but the running states of the target system are unknown due to non-deterministic interrupts, task scheduling and other system activities..
 They are able to
...
We assume an unprivileged attacker that cannot run a processes on the target systems
   to concurrently modify or probe the cache state of the system,
 and they just measure the overall execution time of the block ciphers.
The attackers do not have physical access to the hardware.
...
\begin{itemize}
  ...
  The scheme is algorithm-independent and
        works well for different implementations
...
A naive integration is to perform both {\scshape{Warm}} and {\scshape{Delay}} every time the protected cryptographic function is invoked.
...
similarly,
     {\scshape{Warm}} is performed only when some entries are not in caches.
  ...
  \item \textbf{Case 3}: The execution time of {\scshape{Crypt}} is in $[T_{W}, \infty )$.
   ...
   As explained in Section~\ref{sec:assumption},
     the running states including the system activities,
       ...
       so in this case {\scshape{Delay}} is not performed, too.
       %the attackers cannot find the actual execution time of encryption to infer the keys.
\end{itemize}

When applying the {\scshape{Warm+Delay}} scheme, the remote attackers are (assumed to be) able to measure the time of each  execution of {\scshape{ProtectedCrypt}},
...
In the following,
we further explain that the integration scheme successfully prevent typical remote cache timing side-channel attacks.
