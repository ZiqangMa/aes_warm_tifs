...
The attackers do not have physical access to the hardware. % and cannot run any processes on the target as well.
...
other cache timing side-channels by active attackers {\color{red}(such as xxxx \cite{xx,xx,xxx})} are out of the scope of this paper.
...
A naive integration scheme is to perform both {\scshape{Warm}} and {\scshape{Delay}} every time the protected cryptographic function is invoked.
...
All these operations are algorithms-independent and implementation-transparent,
        except that the memory address of lookup tables is needed in {\scshape{Warm}}.
...
So the differential information is determined by the system events but not the keys.
