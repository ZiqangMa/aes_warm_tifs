...
This section presents the preliminaries of our work. First, we introduce the threat model and the objectives of this paper. We then describe the {\scshape{Warm+Delay}} defense scheme, and show its effectiveness through security analysis. This section provides a foundation for the analysis of performance optimization in Section \ref{sec:performanceproof}.
...
The algorithms of block ciphers and their implementation details are publicly known, but the keys are kept secret, which are the target of the attacks.
In particular,
...
We assume that remote attackers know the software/hardware configurations of the target system, including the operating system, cache hierarchy, etc. However, the running states of the target system are unknown to the attackers, due to non-deterministic interrupts, task scheduling and other system activities.
...
$T_{NM}$ is the time period for one encryption execution,
  in the case that no cache misses occur during the execution, i.e. all lookup tables are in the caches.
