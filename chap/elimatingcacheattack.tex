...
These mechanisms work complementarily and each has its own advantages:
    {\scshape{Warm}} improves the performance, but it cannot completely ensure security by itself
...
%In this algorithm,
There are two parameters, $T_{nm}$ and $T_{mt}$.
...
$T_{mt}$ is defined as the worst execution time for one encryption,
...
Given an implementation of  block ciphers, $T_{nm}$ and $T_{mt}$
...
%Our scheme invokes $Delay$ (line $9$ in Algorithm~\ref{alg:aesdefense})only when the encryption time is larger than the \verb+ENC_THRESHOLD+ but less than T_{mt}(worst execution time). The $Delay$ introduces the time delay to make the encryption time equal to T_{mt} for cache miss confusion.%这两句是对方案进行解释说明，warm只在必要的时候进行，delay进行的条件是大于阈值小于WET，同时说了delay是将所有需要delay的时间延迟到WET。
%]]]]]]]

The execution time of {\scshape{Encrypt}}
falls into three intervals, $(0, T_{nm}]$, $(T_{nm}, T_{mt})$ and $[T_{mt}, \infty )$.
...
\begin{itemize}
  ...
  \item \textbf{Case 2}: The execution time is in $(T_{nm}, T_{mt})$. It will be delayed to $T_{mt}$,
  ...
  \item \textbf{Case 3}: The execution time of {\scshape{Encrypt}} is in $[T_{mt}, \infty )$.
...
When applying the {\scshape{Warm+Delay}} scheme, as shown in Algorithm~\ref{alg:aesdefense}, the remote attackers are (assumed to be) able to measure each execution time of {\scshape{ProtectedEncrypt}},
 ...
 So the measured time, i.e., the execution time of {\scshape{ProtectedEncrypt}},
  falls into two intervals, $(0, T_{nm}]$ and $[T_{mt}, \infty )$.
...
The values of $T_{nm}$ and $T_{mt}$ are irrelevant to the data processed,
...
When the time is greater than $T_{mt}$,
...
When protected by the {\scshape{Warm+Delay}} scheme,
 the execution time measured by attackers is irrelative to the inputs of the lookup table
  and the time variation is caused by the random and unknown system activities.
