...
When the measured time is greater than $T_{w}$,
    it is probably caused by random system activities
    which mask the relationship between the time and data processed.
The measured time is increased by {\scshape{Warm}} when $t2-t1 > T_{nm}$,
    in the case that
        (\emph{a}) almost all entries are into caches after the encryption,
                so the Warm is no time, and Delay is involved to destroy the relationship.
        (\emph{b}) no entry is in caches, then it means that system activities are involved during the encryption.
Although Warm leak information.
...
So the time in these two intervals cannot be exploited in cache timing side-channel attacks.
%    The relationship between the execution time and data processed during encryption is destroyed.
     Thus, the cache timing side-channel attacks do not work under the integration scheme in Algorithm \ref{alg:aesdefense}.
%
