...
Given a block cipher, the execution time without cache misses (or with all tables cached, denoted as $T_{NM}$)
 and the worst execution time with all tables uncached before encryption (and cached during encryption, denoted as $T_{W}$)
  are constant on a certain computing platform.
Note that, during an execution of encryption, some table entries are cached automatically.
...
Three operations in addition to encryption,
...
$T_{NM}$ is defined as the (possible maximal) period for one execution of encryption/decryption,
...
In general,
  {\scshape{ProtectedCrypt()}} are invoked continuously, to process a great deal of data,
...
%\vspace{-1mm}
The remote attackers are (assumed to be) able to measure the time of each  execution of {\scshape{ProtectedCrypt()}}, but not {\scshape{Crypt()}}.
