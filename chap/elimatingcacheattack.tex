...
In particular,
 the implementation of block ciphers is based on lookup tables,
  and the execution time depends mostly on cache access of the lookup tables.
...
That means,
 ...
 so they can continuously invoke the functions to cache
    ...
    or invoke no function for a long time to let
           all tables be evicted, with an extreme high probability.
We assume an unprivileged attacker that cannot run any processes on the target systems to get the middle-state of the ciphers,
...
Given a block cipher, the execution time without cache misses (TWOCM) and the worst execution time (WET)
        ...
        \begin{algorithmic}[1]
           ...
           \Function {ProtectedCrypt}{$key$, $in$, $out$}
                ...
                \If {$end - start > TWOCM$}
                    ...
                    \If {$delay - start < WET $}
                        \State $\Call{Delay}{WET-delay+start}$
                    \EndIf
                \EndIf
           \EndFunction
        \end{algorithmic}
...
In this algorithm,
  there are two constant parameters, WET and TWOCM.
WET is defined as the period for one execution of encryption/decryption,
  in the case that none of lookup tables is cached before the execution.
TWOCM is defined as the (possible maximal) period for one execution of encryption/decryption,
...
\begin{itemize}
  \item \textbf{Case 1}: The execution time of {\scshape{Crypt()}} is in $(0, TWOCM]$.
  ...
  \item \textbf{Case 3}: The execution time is greater than $WET$.
...
However, protected by the {\scshape{Warm+Delay}} scheme,
  any plaintext may be processed in the short time (i.e., TWOCM) due to cache warm,
  while any plaintext may be done in the long time (i.e., WET) due to delay.
