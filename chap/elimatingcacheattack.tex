...
We then describe the {\scshape{Warm+Delay}} scheme, and analyze its effectiveness.
...
\hline
$T_{mm}$ & The encryption execution time with the \underline{m}aximum of cache \underline{m}isses. \\
...
When we consider that the cryptographic function is invoked continuously for large amounts of data,
 the time of performing {\scshape{Warm}} may vary and this variation reflects the cache state after the last encryption.
...
$T_{mm}$ is defined as the time period
    with the \underline{m}aximum of cache \underline{m}isses,
    i.e.,
...
We do not query the status of any cache line to determine whether a table entry is in caches or not,
...
\begin{itemize}
  ...
  \item \textbf{Case 3}: The execution time of is in $[T_{mm}, \infty )$.
...
Although the execution time of {\scshape{Warm}} may leak the cache states after last encryption,
