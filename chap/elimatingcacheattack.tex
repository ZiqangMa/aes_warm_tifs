...
Note that,
  even if this initial {\scshape{Warm}}  is not performed,
   the security is still ensured by {\scshape{Delay}} afterward and
    the impact of performance is  negligible if the loop of {\scshape{ProtectedEncrypt}} is long enough.
%Moreover,
...
\begin{itemize}
  ...
  \item \textbf{Case 2}: The execution time of {\scshape{Encrypt}} is in $(T_{NM}, T_{W})$. It will be delayed to $T_{W}$,
...
When applying the {\scshape{Warm+Delay}} scheme, as shown in Algorithm~\ref{alg:aesdefense}, the remote attackers are (assumed to be) able to measure the time of each  execution of {\scshape{ProtectedEncrypt}},
