...
%\vspace{-2mm}
This section first presents the threat mode and design goals.
...
The attackers are able to
  invoke the encryption/decryption functions arbitrarily
...
The attackers control the invocation,
 so they can continuously invoke the functions to cache
    ...
    or invoke no function for a long time to let
           all tables be evicted from caches, with an extreme high probability.
We assume an unprivileged attacker that cannot run any processes on the target systems
   to concurrently modify (or read) the cache state of the system (shared with the cryptographic engine)
...
The combination of the two strategies follows some principles:
\begin{enumerate}
  \item
