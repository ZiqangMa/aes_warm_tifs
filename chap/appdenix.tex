...
\begin{equation*}
\begin{split}
&P_{S_{\bar{c}, a}|S_{\bar{c},a},\bar{w}}^{\ominus}=P_{evict}^{\mathfrak{3}\mathcal{L}}P_{a}^{\mathfrak{31}\mathcal{L}}+(1-P_{evict}^{\mathfrak{3}\mathcal{L}})P_{a}^{\mathfrak{32}\mathcal{L}} .
\end{split}\end{equation*}
It can be seen from analysis that $P_{a}^{\mathfrak{31}\mathcal{L}} > P_{a|S_{c}}$ and $P_{a}^{\mathfrak{32}\mathcal{L}} > P_{a|S_{c}}$.
So we can get $P_{a}^{\mathfrak{3}\mathcal{L}} > P_{a|S_{c}}$.
In this way,
...
We estimate if it is larger than $0$.
...
&E_{M}=P_{e|S_{c}}+P_{warm}^{\mathcal{M}}+P_{S_{\bar{c}, a}|S_{\bar{c},\bar{a}}}-P_{warm}^{\mathcal{M}}P_{S_{\bar{c}, a}|S_{\bar{c},\bar{a}}} \\& \ \ \ \ \ \ \ \ -P_{e|S_{c}}(P_{warm}^{\mathcal{M}}+P_{a|S_{c}}-P_{warm}^{\mathcal{M}}P_{a}^{\mathfrak{1}})\\
...
\end{equation}
We denote $C_{M}(1-P_{e|S_{c}})P_{S_{\bar{c}, a}|S_{\bar{c},\bar{a}}}+D_{M}$ as $\Omega$.
Therefore,
\begin{equation}
%\label{equ:detail2}
\begin{split}
\Omega &=-(P_{e|S_{c}})^2(P_{a|S_{c}})^2  \\
...
Regard $\Omega$ as the function of $P_{a|S_{c}}$,
and we can find that it is an quadratic function.

When $P_{a|S_{c}} = 1$,
$$\Omega(P_{a|S_{c}})=M(1-P_{e|S_{c}})(P_{S_{\bar{c}, a}|S_{\bar{c},\bar{a}}})^2 \geq 0 \ .$$

Let $\Omega(P_{a|S_{c}}) = 0$,
 and denote the smaller root of the equation as $P_{a\_lroot}^{\mathfrak{1}}$.
  so when $P_{a|S_{c}} \geq P_{a\_lroot}^{\mathfrak{1}}$, $\Omega \geq 0$.
   When $P_{a|S_{c}} < P_{a\_lroot}^{\mathfrak{1}}$, $\Omega < 0$.
 Thus,
\begin{enumerate}
  \item
  When $P_{a|S_{c}} \geq P_{a\_lroot}^{\mathfrak{1}}$, $\Omega \geq 0$.
  ...
  When $P_{a|S_{c}} < P_{a\_lroot}^{\mathfrak{1}}$, $\Omega < 0$.
   ...
   \begin{equation}\label{equ:ge}
     g \leq \frac{C_{M}P_{e|S_{c}}(P_{a|S_{c}}-1)}{\Omega}
   \end{equation}
    for all $P_{e|S_{c}} \in [0,1]$ and $P_{S_{\bar{c}, a}|S_{\bar{c},\bar{a}}} \in [0,1]$.
\end{enumerate}

For the second case, denote
    $$G_e = \frac{C_{M}P_{e|S_{c}}(P_{a|S_{c}}-1)}{\Omega}$$


\begin{enumerate}
\item
    First, we regard $G_e$ as the function of $P_{a|S_{c}}$ and calculate the derivative on $P_{a|S_{c}}$, and get:
    \begin{equation*}
    \begin{aligned}
    &\frac{d(G_e)}{d(P_{a|S_{c}})} = (P_{e|S_{c}})^{3}(1-P_{a|S_{c}})^2\\
    &\ \ \ +M(1-P_{e|S_{c}})P_{S_{\bar{c}, a}|S_{\bar{c},\bar{a}}}(P_{e|S_{c}}+P_{S_{\bar{c}, a}|S_{\bar{c},\bar{a}}}-P_{e|S_{c}}P_{a|S_{c}})^{2}
    \end{aligned}
    \end{equation*}

Therefore,$\frac{d(G_e)}{d(P_{a|S_{c}})} \geq 0$, so that $G_e$ is a monotony increase function of $P_{a|S_{c}}$.
When $P_{a|S_{c}}$ fetches the minimum value, $G_e$ gets its minimum value.

\item
Second, we regard $G_e$ as the function of $P_{S_{\bar{c}, a}|S_{\bar{c},\bar{a}}}$ and calculate the derivative on $P_{S_{\bar{c}, a}|S_{\bar{c},\bar{a}}}$, and get:
\begin{equation*}
\begin{split}
\frac{d(G_e)}{d(P_{S_{\bar{c}, a}|S_{\bar{c},\bar{a}}})} = &\{M(1-P_{e|S_{c}})(1-P_{a|S_{c}})(P_{S_{\bar{c}, a}|S_{\bar{c},\bar{a}}}+P_{e|S_{c}}(1-P_{a|S_{c}}))^2\\
&-(P_{e|S_{c}})^2(1-P_{a|S_{c}})^2 \}P_{e|S_{c}}
\end{split}
\end{equation*}

Denote $P_{a\_rroot}^{\mathfrak{2}}$ as the greater root of equation $\frac{d(G_e)}{d(P_{S_{\bar{c}, a}|S_{\bar{c},\bar{a}}})}=0$.
\begin{equation*}
  P_{a\_rroot}^{\mathfrak{2}} = P_{e|S_{c}}\sqrt{\frac{1-P_{a|S_{c}}}{M(1-P_{e|S_{c}})}}-P_{e|S_{c}}(1-P_{a|S_{c}})
\end{equation*}
So, when $
0 \leq P_{S_{\bar{c}, a}|S_{\bar{c},\bar{a}}} \leq P_{a\_rroot}^{\mathfrak{2}}
$,
$$\frac{d(G_e)}{d(P_{S_{\bar{c}, a}|S_{\bar{c},\bar{a}}})} \leq 0 \ .$$

Therefore, when $P_{a\_rroot}^{\mathfrak{2}} \geq 1$, $G_e$ is a monotony decrease function of $P_{S_{\bar{c}, a}|S_{\bar{c},\bar{a}}}$.
...
\item
 Third, we regard $G_e$ as the function of $P_{e|S_{c}}$ and calculate the derivative on $P_{e|S_{c}}$, and get:
\begin{equation*}
\begin{split}
\frac{d(G_e)}{d(P_{e|S_{c}})} &= P_{S_{\bar{c}, a}|S_{\bar{c},\bar{a}}}(1-P_{a|S_{c}})(1-M+MP_{a|S_{c}})(P_{e|S_{c}})^2\\
&-2MP_{a|S_{c}}P_{S_{\bar{c}, a}|S_{\bar{c},\bar{a}}}(1-P_{S_{\bar{c}, a}|S_{\bar{c},\bar{a}}})P_{e|S_{c}}-M(P_{S_{\bar{c}, a}|S_{\bar{c},\bar{a}}})^3
\end{split}
\end{equation*}
When $P_{e|S_{c}} = 0$, $\frac{d(G_e)}{d(P_{e|S_{c}})} = -M(P_{S_{\bar{c}, a}|S_{\bar{c},\bar{a}}})^2 \leq 0 $.
Let $\frac{d(G_e)}{d(P_{e|S_{c}})} = 0$, we can get the right root of this equation:
\begin{equation}
\begin{split}
P_{evict\_rroot} &= \frac{MP_{a|S_{c}}(1-P_{S_{\bar{c}, a}|S_{\bar{c},\bar{a}}})+\sqrt{\Delta}}{(1-P_{a|S_{c}})(1-M+MP_{a|S_{c}})}
\end{split}
\end{equation}
where
\begin{align}
...
So when
$
0 \leq P_{e|S_{c}} \leq P_{evict\_rroot}
$,
$$\frac{d(G_e)}{d(P_{e|S_{c}})} \leq 0\ .$$

Therefore, when $P_{evict\_rroot} \geq 1$, $G_e$ is a monotony decrease function of $P_{e|S_{c}}$.
