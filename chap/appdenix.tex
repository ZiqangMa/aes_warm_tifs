...
By combing Equations~\ref{equ:pic} and \ref{equ:pil} in Equation \ref{equ:tpc1},
...
& - P_{e|S_{c}}P_{S_{\bar{c}, a}|S_{\bar{c},\bar{a}}}(1+P_{S_{\bar{c}, a}|S_{\bar{c},\bar{a}}}^{\ominus}-P_{a}^{\mathfrak{3}\mathcal{L}}), \end{split}
\end{equation*}
\begin{equation*}
\begin{split}
...
\textcolor[rgb]{1.00,0.00,0.00}{It can be seen from analysis that $P_{a|S_{\bar{c}, a}}^{\ominus} > P_{a|S_{c}}$.}
Meanwhile, in the Markov state transition diagram,
\begin{equation*}
\begin{split}
...
\textcolor[rgb]{1.00,0.00,0.00}{We find that }$P_{S_{\bar{c}, a}|S_{\bar{c},\bar{a}}^{\bar{w}}}^{\oplus} = P_{S_{\bar{c}, a}|S_{\bar{c},\bar{a}}}$.
Then by combing Equations~\ref{equ:pic} and \ref{equ:pim}, we get Equations~\ref{equ:ypwarm2} and \ref{equ:detail2}.
...
Therefore,$\frac{d(F_G)}{d(P_{a|S_{c}})} \geq 0$, so that $F_G$ is a monotony increasing function of $P_{a|S_{c}}$.
When $P_{a|S_{c}}$ fetches the minimum value, $F_G$ gets its minimum.
...
Therefore, when $P_{S_{\bar{c}, a}|S_{\bar{c},\bar{a}},rroot} \geq 1$, $F_G$ is a monotony decreasing function of $P_{S_{\bar{c}, a}|S_{\bar{c},\bar{a}}}$.
...
\label{appendixb}
The size of a cache line is $C$ bytes. $P_{a}$ represents that the probability that some of the $N$ cache line size of lookup tables not in caches are accessed. When the AES implementation uses 4.25KB lookup tables, $P_{a}$ for AES-128, AES-192 and AES-256 is as follows:
\begin{align}
%\begin{flalign}
%\begin{split}
    P_{a}^{[128]} = &1-   \nonumber \\
