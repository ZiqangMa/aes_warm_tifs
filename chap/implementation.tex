...
As we attempt to optimize performance while eliminate remote cache timing side channels,
 efficient {\scshape{Warm}}, {\scshape{GetTime}},  and {\scshape{Delay}}
  are finished;
and the constant parameters ($T_{nm}$ and $T_{mt}$) are determined properly.
...
so,  we perform {\scshape{GetTime}} only if $t2 - t1 < T_{mt}$,
...
\vspace{1mm}\noindent\textbf{$\mathbf{T_{mt}}$.}
$T_{mt}$ is the worst AES execution time and unrelated to the cache states.
Before measuring, we flush both the data and instructions out of L1/2/3 caches.
In actual measurement, we repeat the measurement for 10000 times, and calculate the average value of 100 greater measurements as   $T_{mt}$.
 In this case $T_{mt}$ is 2834 CPU cycles.
