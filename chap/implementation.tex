...
As we attempt to optimize the performance while eliminate remote cache timing side channels,
 efficient {\scshape{Warm}}, {\scshape{GetTime}},  and {\scshape{Delay}}
  are finished;
and the constant parameters ($T_{nm}$ and $T_{mm}$) are set properly.
...
In the following experiments, Hyper-Threading is disabled.
...
\centering
\subfloat[in normal case]{\includegraphics[width=0.23\textwidth]{pic/fwnormal.pdf}
\label{fig:fullwarmworst}}
\hfil
\subfloat[the best case]{\includegraphics[width=0.23\textwidth]{pic/fwbest.pdf}
\label{fig:fullwarmbest}}
...
Moreover, the cost of \verb+RDTSCP+ is 36 cycles which is much greater than the comparison operation and cannot be ignored.
...
The average minimal AES execution time is measured by average $2^{30}$ AES execution time with the lookup tables all in L1D cache.
In our environment it is $331$ cycles.
 \figurename~\ref{fig:twocm} shows the distribution of AES execution time for $2^{30}$ plaintexts
   while all table entries except one block of 64 bytes (i.e., one cache line) are loaded in L1D cache.
Note that, this uncached entry may be unnecessary in an execution of AES encryption.
The stack switch technique makes the AES execution time much more concentrated,
 which helps the measurement of $T_{nm}$ to be more accurate and reasonable.
  Also we choose the value as large as possible to avoid the influence of the fluctuation around $T_{nm}$ that might occur.
Finally, we choose 355 cycles as $T_{nm}$.
This value is chosen to ensure that all table entries are in L1D caches and no fluctuation occurs around it. So no useful observation will be obtained by attackers, and no useful variations will be magnified.


\vspace{1mm}\noindent\textbf{$\mathbf{T_{mm}}$.}
$T_{mm}$ is the maximum encryption execution time when all table entries are out of caches and unrelated to the cache states.
Before measuring, we flush both the data and instructions out of L1/2/3 caches.
We repeat the measurement for 10000 times, and calculate the average value of \textcolor[rgb]{1.00,0.00,0.00}{100 greatest measurements} as $T_{mm}$.
...
As the effect of instruction caches on the execution time is almost indistinguishable from that of data caches,
  we measure $T_{nm}$ when all instructions of encryption are cached
