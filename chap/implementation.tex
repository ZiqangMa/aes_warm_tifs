...
\figurename~\ref{fig:fullwarmworst} shows the distribution of the execution time for $2^{30}$ random plaintexts.
...
We measure the time for different loop number of the \verb+xor+ instruction, by invoking it $10^6$ times with different loop numbers (from 0 to 2000 and the step is 50), as shown in \figurename~\ref{pic:delay}.
The relation between the time delayed ($T_d$) and the loop number of \verb+xor+ instruction ($n$) is calculated through the least squares method. The result is $t_{delay} = 2.995n + 12.886$, and the coefficient of determination is 0.999993.
