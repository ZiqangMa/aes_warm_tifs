...
However, even when all lookup tables are in the L1 cache,
  the execution time of encryption still has variations.
Figure~\ref{fig:fullwarmese-diffinput} shows the distribution of the execution time,
   xxxxxxxxxxx.
...
These variations could be exploited to launched side-channel attackers~\cite{Canteaut2006Understanding,Bernstein2005Cache}.
...
Finally,
 the distribution of the AES encryption time for $2^{30}$ random plaintexts is shown in Figure~\ref{fig:fullwarmbest}.
...
The \verb+usleep()+ and \verb+nanosleep()+ are inappropriate, as they don't have enough accuracy, and they switch the state of AES execution process to {TASK\_INTERRUPTIBLE}, which may make the lookup tables evicted from caches. increase the probability.
We perform the {\vdelay} operation by executing xor instruction repeatedly, achieving a high precision without modifying the cache state.
